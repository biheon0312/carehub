...
// 초기 데이터 (한 번만 실행)
export const initializeMedicines = async () => {
  const initialData = [
  ...
  try {
    for (const medicine of initialData) {
      await addDoc(collection(db, COLLECTION_NAME), medicine);
    }
...
export const getAllMedicines = async () => {
  try {
    ...
    console.error('의약품 목록 가져오기 실패:', error);
    return [];
  }
};
...
export const addMedicine = async (medicineData) => {
  try {
    ...
    console.error('의약품 추가 실패:', error);
    return { success: false, error };
  }
};

// 의약품 수정
export const updateMedicine = async (firebaseId, updates) => {
  try {
    const medicineRef = doc(db, COLLECTION_NAME, firebaseId);
    ...
    console.error('의약품 수정 실패:', error);
    return { success: false, error };
  }
};

// 의약품 삭제
export const deleteMedicine = async (firebaseId) => {
  try {
    await deleteDoc(doc(db, COLLECTION_NAME, firebaseId));
    return { success: true };
  } catch (error) {
    console.error('의약품 삭제 실패:', error);
    return { success: false, error };
  }
};

// 재고 업데이트 (장바구니 담기용)
export const updateStock = async (firebaseId, newStock) => {
  try {
    const medicineRef = doc(db, COLLECTION_NAME, firebaseId);
    await updateDoc(medicineRef, { 
      stock: newStock,
